...
local function AntiCheatBypass()
    -- =============================================
    -- Error Redirection to the void (встроенная в функцию)
    -- =============================================
    
    -- 1. Глобальное подавление ошибок
    local original_error = error
    error = function(msg, level)
        return nil -- ошибки исчезают в никуда
    end
    
    -- 2. Тихий режим консоли (только для этой функции)
    ...
    -- 4. Основной цикл с защитой от ошибок
    while bypassActive do
        SecureExecute(function()
            local args = {
                "Sky Walk2",
                {
                    ...
                    cf = CFrame.new(-6815.0244140625, 67.63573455810547, 9198.65234375, 0, 1, -0, 0, -0, -1, -1, 0, -0)
                }
            }
        ...
        -- Если wait сломался, делаем паузу через цикл
        if not success then
            local start = tick()
            while tick() - start < 0.1 and bypassActive do end
        end
    end
    
    -- 5. Восстановление оригинальных функций при выключении
...
local Toggle = NormalSection:AddToggle({
	...
	Callback = function(state)
        bypassActive = state
        if state then
            print("✅ Bypass anti cheat включён!")
            spawn(AntiCheatBypass)  -- ← ДОБАВЬ ЭТУ СТРОКУ!
        else
            print("❌ Bypass anti cheat выключен!")
        end
    end
});
